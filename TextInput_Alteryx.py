    ...
    for i in tree.iter():
        if(i.tag=='Node'): 
            ...
            if(len(list(filter(lambda x:x.tag=='GuiSettings',list(i.iter()))))>0):
                ...
                if(str(plugin)!='None' and 'TextInput' in plugin):
                    ...
                    for j in i.iter():
                        if(j.tag=='Position'):
                            ...
                            no_of_rows=j.attrib.get("value")
                        elif(j.tag=='Field'):
                            Field_name=j.attrib.get("name")
                    ss=filter(lambda x:x.tag=='Data',list(tree.iter()))
                    result=list(ss)
                    for data_index in range(0,len(result)):


                        data_text=result[data_index].text
                        row_list=result[data_index].findall("r")

                        for row_index in range(0,len(row_list)):
                            row_text=row_list[row_index].text
                            col_list=row_list[row_index].findall("c")
                            for col_index in range (0,len(col_list)):
                                col_text=col_list[col_index].text
                                ...
                                df=df.append(pd.DataFrame({'Filename':filename.rsplit('/')[-1].replace('.XML',''), 'ToolID': ToolID,
                                                                        ...
                                                                        'Expression_1':str(data_text),
                                                                        'Expression_2':str(row_text),
                                                                        'Expression_3':str(col_text),
                                                                        'GroupBy':str(GroupBy),
