    ...
    for i in tree.iter():
        if(i.tag=='Node'): 
            ...
            if(len(list(filter(lambda x:x.tag=='GuiSettings',list(i.iter()))))>0):
                ...
                if(str(plugin)!='None' and 'Formula' in plugin):
                    ...
                    for j in i.iter():
                        if(j.tag=='Position'):
                        ...
                        elif(j.tag=='FormulaField'):
                            field_name=j.attrib.get("field")
                            field_expression=j.attrib.get("expression")
                            field_size=j.attrib.get("size")
                            field_type=j.attrib.get("type")
                            df=df.append(pd.DataFrame({'Filename':filename.rsplit('/')[-1].replace('.XML',''),  'ToolID': ToolID,
                                                    ...
                                                    'Expression_1':str(field_expression),
