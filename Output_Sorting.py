...
def append_to_linked_list(list,detailed_df,target_list,filename):
    ...
    for src_target in list:
        ...
        if(str(src_target[1]) not in target_list):
            ...
            target_plugin = target_subset['Target Plugin'].drop_duplicates()
            for index in target_plugin.index:
                target_df=return_dict(str(target_plugin[index]), filename)
                target_df.drop('FileName',inplace=True,axis=1)
                target_df.to_csv('Outputdbfile.csv',mode='a',header=True,index=True)
                base_target_slice=pd.DataFrame(data=[[filename,str(src_target[1]),str(target_plugin[index]),'NA','NA']],columns=[str(filename),str(src_target[1]),str(target_plugin[index]),"NA","NA"])
    ...
    sorted_df=pd.read_csv("New.csv",
                    names=["Input File Name","Source ID","Source Plugin","Target ID","Target Plugin","Plugin", "ToolID","FieldName","Rename_or_Destination","CreateFieldName","UpdateFieldName","LeftJoinField","RightJoinField","Expression_1","Expression_2","Expression_3","GroupBy","SortOrder","UpdateField","MultifileValue","HeaderField","DataField","IsSelected","NoOfRows","NumFields","ErrorHandlingText"],keep_default_na=False)
...
def sort_output(filename1,filename2,xmlfilename):
    ...
    list2=[]
    append_to_linked_list(list,detailed_df,target_list,xmlfilename)


sort_output("Output.csv","test3.csv","03_TAG_description_master_data.yxmd")