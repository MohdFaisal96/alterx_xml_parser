    ...
    for i in tree.iter():
        ...
        if(i.tag=='Node'): 
            ...
            if(len(list(filter(lambda x:x.tag=='GuiSettings',list(i.iter()))))>0):
                ...
                if(str(plugin)!='None' and 'Sample' in plugin):
                    ...
                    if(len(list(filter(lambda x:x.tag=='Mode',list(i.iter()))))>0):
                        ss=filter(lambda x:x.tag=='Mode',list(i.iter()))
                        result=list(ss)
                        Mode_Value=result[0].text
                    if(len(list(filter(lambda x:x.tag=='N',list(i.iter()))))>0):
                        ss=filter(lambda x:x.tag=='N',list(i.iter()))
                        result=list(ss)
                        N_Value=result[0].text
                    if(len(list(filter(lambda x:x.tag=='GroupFields',list(i.iter()))))>0):
                        ss=filter(lambda x:x.tag=='GroupFields',list(i.iter()))
                        result=list(ss)
                        orderChanged_Value=result[0].attrib.get("orderChanged")
                        for data_index in range(0,len(result)):
                            data_text=result[data_index].text
                            field_list=result[data_index].findall("Field")
                            for row_index in range(0,len(field_list)):
                                field_name=field_list[row_index].attrib.get("name")
